...
Owns every read/write of `IndexCollection` and `Document`'s indexing-metadata columns, plus the
one dimension-compatibility check every write/search path must pass through before touching
Qdrant. `IngestionWorker` (write side) and `app.services.indexing.reindex_service` both call
`ensure_active_collection()` before upserting; nothing else creates a Qdrant collection or decides
whether a document is stale.
"""
...
async def retire_collection(session: AsyncSession, collection_name: str) -> None:
    ...
    Explicit cleanup boundary: retiring a collection here is a bookkeeping step only. Actually
    removing the collection from Qdrant is a separate, deliberate operational action this
